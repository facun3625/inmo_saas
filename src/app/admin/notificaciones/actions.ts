...
export async function sendCustomerBroadcast(formData: FormData) {
  try {
    return await runSendCustomerBroadcast(formData);
  } catch (err) {
    return toUserError(err, "No se pudo enviar la notificación");
  }
}

async function runSendCustomerBroadcast(formData: FormData): Promise<{ sent: number }> {
  const { tenant, features } = await requireTenantAdminWithPlan();
  if (!features.allowPushNotifications) throw new ActionError("Tu plan no incluye notificaciones push");
  ...
  if (await isRateLimited(rateLimitKey, PUSH_BROADCAST_RULE)) {
    throw new ActionError("Ya mandaste varias campañas esta hora — probá de nuevo más tarde.");
  }
