...
export async function createCoupon(formData: FormData) {
  try {
    return await runCreateCoupon(formData);
  } catch (err) {
    return toUserError(err, "No se pudo guardar el cupón");
  }
}

async function runCreateCoupon(formData: FormData) {
  ...
  if (parsed.discountType === "PERCENT" && parsed.discountValue > 100) {
    throw new ActionError("Un descuento porcentual no puede ser mayor a 100");
  }
...
export async function updateCoupon(id: string, formData: FormData) {
  try {
    return await runUpdateCoupon(id, formData);
  } catch (err) {
    return toUserError(err, "No se pudo guardar el cupón");
  }
}

async function runUpdateCoupon(id: string, formData: FormData) {
  const { tenant } = await requireTenantAdmin();
  ...
  if (existing) throw new ActionError("Ya existe otro cupón con ese código");
...
export async function setCouponEnabled(id: string, enabled: boolean) {
  try {
    return await runSetCouponEnabled(id, enabled);
  } catch (err) {
    return toUserError(err, "No se pudo guardar el cupón");
  }
}

async function runSetCouponEnabled(id: string, enabled: boolean) {
  const { tenant } = await requireTenantAdmin();
...
export async function deleteCoupon(id: string) {
  try {
    return await runDeleteCoupon(id);
  } catch (err) {
    return toUserError(err, "No se pudo guardar el cupón");
  }
}

async function runDeleteCoupon(id: string) {
  const { tenant } = await requireTenantAdmin();
  const redemptionCount = await prisma.couponRedemption.count({ where: { couponId: id } });
  if (redemptionCount > 0) {
    throw new ActionError("No se puede borrar un cupón ya usado. Desactivalo en su lugar.");
  }
