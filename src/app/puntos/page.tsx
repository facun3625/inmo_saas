...
export default async function PuntosPage() {
  ...
  const redeemable = coupons.filter(
    (c) => !c.usageLimit || c._count.redemptions < c.usageLimit,
  );
                    ...
                    {pendingRedemptions.map((r) => (
                      ...
                      >
                        ...
                        <span className="text-muted-foreground">
                          {formatDiscount(
                            r.coupon.discountType,
                            Number(r.coupon.discountValue),
                          )}{" "}
              ...
              {hasPendingRedemption && (
                <p className="rounded-2xl border border-primary/30 bg-primary/5 px-4 py-3 text-sm text-muted-foreground">
                  ...
                  <span className="font-medium text-foreground">
                    {pendingRedemptions[0].coupon.code}
                  </span>{" "}
                  canjeado sin usar — usalo en un pedido antes de canjear otro.
                </p>
              )}
              {redeemable.length === 0 ? (
                    ...
                    >
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span className="text-xs text-muted-foreground">
                          ...
                          {c.expiresAt &&
                            ` · vence ${dateFormatter.format(c.expiresAt)}`}
                        </span>
                      </div>
                      <div className="flex shrink-0 items-center gap-3">
                        ...
                        <RedeemCouponButton
                          couponId={c.id}
                          disabled={
                            hasPendingRedemption || balance < c.pointsCost
                          }
                        />
                      </div>
                    </div>
