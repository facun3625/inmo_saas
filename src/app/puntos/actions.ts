...
export async function redeemCoupon(couponId: string) {
  try {
    return await runRedeemCoupon(couponId);
  } catch (err) {
    return toUserError(err, "No se pudo canjear el cupón");
  }
}

async function runRedeemCoupon(couponId: string) {
  const session = await auth();
  if (!session?.user) throw new ActionError("Iniciá sesión para canjear puntos");
  const userId = session.user.id;
  const tenantId = session.user.tenantId;
  if (!tenantId) throw new ActionError("No autorizado");

  await prisma.$transaction(async (tx) => {
    const coupon = await tx.coupon.findUnique({ where: { id: couponId, tenantId } });
    if (!coupon || !coupon.active || coupon.pointsCost <= 0) {
      throw new ActionError("Ese cupón no está disponible para canjear");
    }
    ...
    if (balance < coupon.pointsCost) {
      throw new ActionError("No tenés puntos suficientes para este cupón");
    }
